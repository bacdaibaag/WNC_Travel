...
<body>
        ...
        <div class="login-left">
            ...
            <form action="{{ route('updateProfile') }}" method="POST" id="registerForm" class="login-form" autocomplete="off">
                ...
                <div class="login-content">
                    ...
                    <div>
                        <label for="detailAddress">Detail Address</label>
                        <input type="text" id="detailAddress" name="detailAddress" value="{{ Auth::user()->address->detail_address ?? '' }}">
                    </div>
                    <button type="button" id="editBtn" onclick="enableEditing()">Edit</button>
                    <button type="submit" id="saveBtn" style="display:none;">Save</button>
                </div>
    ...
    <script>
        ...
        var districts = document.getElementById("district");
        var wards = document.getElementById("ward");
        var selectedCity = "{{ Auth::user()->address->city ?? '' }}";
        var selectedDistrict = "{{ Auth::user()->address->district ?? '' }}";
        var selectedWard = "{{ Auth::user()->address->ward ?? '' }}";
        var Parameter = {
            url: "https://raw.githubusercontent.com/kenzouno1/DiaGioiHanhChinhVN/master/data.json",
            method: "GET",
            responseType: "application/json",
